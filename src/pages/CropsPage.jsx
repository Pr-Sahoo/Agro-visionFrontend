...
//   const blank = { cropName: "", cropType: "", variety: "", area: "", areaUnit: "acres", soilType: "", plantingDate: "", harvestTarget: "", lat: "", lon: "", city: "", notifyEmail: true, notifySMS: false };
...
//       pos => { setF(p => ({ ...p, lat: pos.coords.latitude.toFixed(6), lon: pos.coords.longitude.toFixed(6) })); setLocLoad(false); },
//       ()  => setLocLoad(false)
//     );
//   };
...
//         cropName: f.cropName, cropType: f.cropType, variety: f.variety,
//         area: parseFloat(f.area) || undefined, areaUnit: f.areaUnit, soilType: f.soilType,
//         plantingDate: f.plantingDate || undefined, harvestTarget: f.harvestTarget || undefined,
//         location: { lat: parseFloat(f.lat), lon: parseFloat(f.lon), city: f.city },
//         notifyEmail: f.notifyEmail, notifySMS: f.notifySMS,
//       });
//       onSuccess();
...
//         <Input label="Crop Name *" placeholder="Wheat, Rice…" value={f.cropName} onChange={s("cropName")} required />
//         <Select label="Crop Type" value={f.cropType} onChange={s("cropType")}>
//           <option value="">Select…</option>
//           {["Cereal","Vegetable","Fruit","Legume","Oilseed","Spice","Fiber"].map(t => <option key={t} value={t}>{t}</option>)}
//         </Select>
//       </div>
//       <div className="grid grid-cols-2 gap-4">
//         <Input label="Variety" placeholder="Basmati, Hybrid…" value={f.variety} onChange={s("variety")} />
//         <Select label="Soil Type" value={f.soilType} onChange={s("soilType")}>
//           <option value="">Select…</option>
//           {["Sandy","Loamy","Clay","Silt","Peaty","Black Cotton"].map(t => <option key={t} value={t}>{t}</option>)}
//         </Select>
//       </div>
//       <div className="grid grid-cols-3 gap-3">
//         <div className="col-span-2"><Input label="Area" type="number" step="0.1" placeholder="2.5" value={f.area} onChange={s("area")} /></div>
...
//       </div>
//       <div className="grid grid-cols-2 gap-4">
//         <Input label="Planting Date" type="date" value={f.plantingDate} onChange={s("plantingDate")} />
//         <Input label="Harvest Target" type="date" value={f.harvestTarget} onChange={s("harvestTarget")} />
//       </div>
...
//           <input type="checkbox" checked={f.notifyEmail} onChange={s("notifyEmail")} className="w-4 h-4 accent-leaf-500" />
//           Email alerts
//         </label>
//         <label className="flex items-center gap-2 cursor-pointer text-sm text-[var(--text)]">
//           <input type="checkbox" checked={f.notifySMS} onChange={s("notifySMS")} className="w-4 h-4 accent-leaf-500" />
//           SMS alerts
//         </label>
//       </div>
...
//         <Btn type="submit" className="flex-1" loading={loading}><Sprout className="w-4 h-4" /> Add Crop</Btn>
//       </div>
//     </form>
//   );
// }
...
//   const days = crop.harvestTarget ? Math.ceil((new Date(crop.harvestTarget) - Date.now()) / 86400000) : null;

//   const remove = async (e) => {
//     e.preventDefault();
//     if (!confirm(`Remove "${crop.cropName}"?`)) return;
//     setDel(true);
//     try { await cropAPI.remove(crop._id); onDelete(crop._id); }
//     catch { setDel(false); }
//   };
...
//             <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-[0_4px_18px_rgba(42,154,42,0.3)]"
//               style={{ background: "linear-gradient(135deg,#3fb83f,#1e7a1e)" }}>
//               <Sprout className="w-6 h-6" />
//             </div>
//             <div>
//               <h3 className="font-semibold text-[var(--text)] group-hover:text-leaf-600 dark:group-hover:text-leaf-400 transition-colors">{crop.cropName}</h3>
//               {crop.variety && <p className="text-xs text-[var(--muted)]">{crop.variety}</p>}
//             </div>
//           </div>
...
//             { label: "Harvest",  val: crop.harvestTarget ? new Date(crop.harvestTarget).toLocaleDateString("en-IN", { day:"numeric", month:"short" }) : "—" },
//           ].map(({ label, val }) => (
//             <div key={label}>
...
//             {days !== null && <Badge variant={days < 0 ? "muted" : days < 14 ? "amber" : "green"}>{days < 0 ? "Past due" : `${days}d to harvest`}</Badge>}
//           </div>
...
//     setLoading(true);
//     try { const { crops } = await cropAPI.getAll(); setCrops(crops); }
...
//           <p className="text-[var(--text2)] text-sm mt-1">{crops.length} crop{crops.length !== 1 ? "s" : ""} being monitored</p>
//         </div>
...
//           <input className="field pl-10" placeholder="Search crops…" value={search} onChange={e => setSearch(e.target.value)} />
//         </div>
//       )}

//       {loading
//         ? <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">{Array(6).fill(0).map((_, i) => <Skel key={i} className="h-56" />)}</div>
//         : filtered.length === 0
//           ? <div className="card p-16 text-center animate-fade-in">
//               <Sprout className="w-16 h-16 text-[var(--muted)] mx-auto mb-5" />
//               <h2 className="font-display text-2xl font-semibold text-[var(--text)] mb-2">{search ? "No crops found" : "No crops yet"}</h2>
//               <p className="text-[var(--muted)] text-sm mb-8 max-w-xs mx-auto">
//                 {search ? `No results for "${search}"` : "Add your first crop to start AI-powered monitoring."}
//               </p>
//               {!search && <Btn onClick={() => setModal(true)} size="lg"><Plus className="w-4 h-4" /> Add First Crop</Btn>}
//             </div>
//           : <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
//               {filtered.map((c, i) => (
//                 <CropCard key={c._id} crop={c} i={i}
//                   onDelete={(id) => { setCrops(p => p.filter(x => x._id !== id)); flash("Crop removed.", "info"); }} />
//               ))}
//             </div>
//       }

//       <Modal open={modal} onClose={() => setModal(false)} title="Add New Crop" size="lg">
//         <CropForm onSuccess={() => { setModal(false); flash("Crop added!"); load(); }} onClose={() => setModal(false)} />
//       </Modal>
